...
def rotate_envmap(shcoeff, angle, lmax=2):
    dj = pyshtools.rotate.djpi2(lmax)
    rotate_shcoeff = SHRotateRealCoef(shcoeff, [angle, 0, 0], dj)
    return rotate_shcoeff
...
def main():
    ...
    tonemapper = TonemapHDR(gamma=2.4, percentile=60, max_mapping=0.6)
    ...
    image = Image.open(f"{root_dir}/{image_dir}/{scene}/dir_0_mip2.jpg").convert("RGB")
