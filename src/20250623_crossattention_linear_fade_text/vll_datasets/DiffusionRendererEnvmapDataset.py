...
class DiffusionRendererEnvmapDataset(torch.utils.data.Dataset):
    ...
    def get_envmap_feature(self, filename, dir_name='envmap_feature'):
        # read file
        envmap_feature_path = os.path.join(self.root_dir, dir_name, filename+'.safetensors')
        if not os.path.exists(envmap_feature_path):
            raise FileNotFoundError(f"Envmap feature file not found: {envmap_feature_path}")
        envmap_feature = load_file(envmap_feature_path) 
        # convert to torch tensor
        envmap_feature = {k: v.to('cpu') for k, v in envmap_feature.items()}
        # convert to list of tensors
        envmap_feature = [envmap_feature[k] for k in sorted(envmap_feature.keys())]  # Sort keys to maintain order
        return envmap_feature  # List of tensors, each tensor is a layer of the environment map feature
    ...
    def get_item(self, idx):
        ...
        if 'envmap_feature' in self.components:
            light_feature = self.get_envmap_feature(filename)
            for layer in range(len(light_feature)):
                output[f'envmap_feature_layer{layer}'] = light_feature[layer]
