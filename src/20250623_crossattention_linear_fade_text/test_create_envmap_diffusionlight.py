...
def from_catecian_to_theta_phi(light_dir):
    """
    Convert light direction from Cartesian coordinates to spherical coordinates (theta, phi).
    input: light_dir (np.array) - [..., 3] in x-right,y-up,z-to_scene convention
    output: theta (np.array), phi (np.array) - spherical coordinates
    """
    # normalize vector to size 1 
    light_dir = light_dir / np.linalg.norm(light_dir, axis=-1, keepdims=True)

    x, y, z = light_dir[..., 0], light_dir[..., 1], light_dir[..., 2]

    theta = np.arctan2(y, x)  # azimuthal angle
    phi = np.arcsin(z)  # polar angle

    # theta is in range [-pi, pi], phi is in range [-pi/2, pi/2]
    # we change theta to be in range [0, 2pi]
...
def main():
    ...
    theta, phi = from_catecian_to_theta_phi(envmap)
