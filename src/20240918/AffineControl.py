...
class AffineControl(L.LightningModule):
    ...
    def generate_tensorboard(self, batch, batch_idx, is_save_image=False, is_seperate_dir_with_epoch=False):
        ...
        tb_name = f"{batch['name'][0].replace('/','-')}/{batch['word_name'][0].replace('/','-')}"
        ...
        if is_save_image:
            ...
            if hasattr(batch, "target_ldr_envmap"):
                ...
                torchvision.utils.save_image(batch['target_ldr_envmap'], f"{self.logger.log_dir}/{epoch_text}target_ldr_envmap/{filename}.jpg")
            if hasattr(batch, "target_norm_envmap"):
                ...
                torchvision.utils.save_image(batch['target_norm_envmap'], f"{self.logger.log_dir}/{epoch_text}target_norm_envmap/{filename}.jpg")
