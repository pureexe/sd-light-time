...
class AffineDeepFloyd(AffineControl):
    ...
    def setup_sd(self):
        if MASTER_TYPE == torch.float16:
            self.pipe = IFImg2ImgPipeline.from_pretrained("DeepFloyd/IF-I-XL-v1.0", variant="fp16", torch_dtype=MASTER_TYPE, watermarker=None)
            # self.pipe_upscale =  IFSuperResolutionPipeline.from_pretrained(
            #     "DeepFloyd/IF-II-L-v1.0", text_encoder=None, variant="fp16", torch_dtype=MASTER_TYPE, watermarker=None
            # )
            self.pipe.watermarker = None    
            #self.pipe_upscale.watermarker = None
            #self.pipe_upscale.enable_model_cpu_offload()
        else:
            self.pipe = IFImg2ImgPipeline.from_pretrained("DeepFloyd/IF-I-XL-v1.0")
            # self.pipe_upscale = IFSuperResolutionPipeline.from_pretrained(
            #     "DeepFloyd/IF-II-L-v1.0", text_encoder=None, watermarker=None
            # )
            self.pipe.watermarker = None    
            # self.pipe_upscale.watermarker = None
            # self.pipe_upscale.enable_model_cpu_offload()
