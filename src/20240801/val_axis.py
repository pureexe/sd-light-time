...
import argparse
from constants import DATASET_ROOT_DIR, VAL_Z_ROOT_DIR
...
LRS = ['1e-4', '1e-4', '5e-5','5e-4', '1e-4', '5e-5', '5e-4', '1e-5', '1e-4', '5e-5', '1e-5', '5e-4', '1e-4', '5e-5', '1e-5', '5e-4']
NAMES = ['vae'] * 20
...
for version in VERSIONS:
    ...
    for chk_pt in CHK_PTS:
        CKPT_PATH = f"output/20240801/multi_mlp_fit/lightning_logs/version_{version}/checkpoints/epoch={chk_pt:06d}.ckpt"
        ...
        if ENV_MODE == "light_direction":
            ...
            if True:
                trainer = L.Trainer(max_epochs=1000, precision=16, check_val_every_n_epoch=1, default_root_dir=f"output/20240801/val_{NAME}_sunrise/{GUIDANCE_SCALE}/{NAMES[version]}/{LRS[version]}/chk{chk_pt}/")
            ...
            if True:
                trainer = L.Trainer(max_epochs=1000, precision=16, check_val_every_n_epoch=1, default_root_dir=f"output/20240801/val_rand100_{VAL_MODE}/{GUIDANCE_SCALE}/{NAMES[version]}/{LRS[version]}/chk{chk_pt}/")
