...
class EnvmapAffineDataset(torch.utils.data.Dataset):
    ...
    def get_normalized_hdr(self, idx):
        hdr_path_jpg = os.path.join(self.root_dir, "exr", self.subdirs[idx], f"{self.files[idx]}.jpg")
        hdr_path_png = os.path.join(self.root_dir, "exr", self.subdirs[idx], f"{self.files[idx]}.png")
        if (not os.path.exists(hdr_path_jpg)) and (not os.path.exists(hdr_path_png)):
        ...
        elif os.path.exists(hdr_path_jpg):
            hdr = torchvision.io.read_image(hdr_path_jpg) / 255.0
        elif os.path.exists(hdr_path_png):
            hdr = torchvision.io.read_image(hdr_path_png) / 255.0
        hdr = hdr[:3]
