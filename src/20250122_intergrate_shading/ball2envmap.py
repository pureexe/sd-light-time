...
def process_image(args: argparse.Namespace, file_name: str):
    ...
    reflect_vec = get_cartesian_from_spherical(env_grid[...,0], env_grid[...,1]) # Z-up
    normal = get_normal_vector(I[None,None], reflect_vec)
    
    # turn from normal map to position to lookup [Range: 0,1]
    pos = (normal + 1.0) / 2

    pos = pos[...,1:] # We ignore X axis because it represent forward.

    # since Z-UP (top 1, bottom -1) but torch grid sample is top-1 bottom 1, we flip only x-axis
    pos[...,1] = 1.0 - pos[...,1]
