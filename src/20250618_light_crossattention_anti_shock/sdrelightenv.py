...
class SDRelightEnv(L.LightningModule):
    ...
    def setup_attention_processor(self):
        ...
        attention_light_gate[0] = 1.0 # set first token to 1.0, this is the token from text prompt
        ...
        for attn_name in self.pipe.unet.attn_processors.keys():
            if attn_name.endswith("attn1.processor"): # first attention is self attention
                ...
                if names[0] == 'up_blocks':
                    ...
                    depth = 3 - int(names[1])
                attn_processor = LightAttentionProcessor(processor_id=light_processor_count, block_depth=depth, token_lengths=token_lengths)
                light_processor_count += 1
    ...
    def get_light_embedings(self, light_features):
        ...
        light_out_embedings = []
        for i, embed in enumerate(light_embedings): 
            # embeding [B, C, H, W]
    ...
    def get_query_embedings(self, text_embedings,light_embedings):
        ...
        query_embedings = torch.cat([
            ...
            self.attention_light_gate[None, None].expand(batch_size, -1, -1),
            light_embedings
        ], dim=1)
    ...
    def configure_optimizers(self):
        ...
        optimizer = optimizer_class([
            {'params': self.attention_light_gate, 'lr': self.learning_rate},  # Unet parameters
