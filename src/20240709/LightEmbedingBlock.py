...
class GateConvBlock(torch.nn.Module):

    def __init__(self,  in_dim=4, out_dim=320, *args, **kwargs):
        super().__init__()
        # possible posenc [nerf:10]
        self.conv_in = torch.nn.Conv2d(in_dim, out_dim, kernel_size=3, stride=1, padding=1)
        self.conv_cond = torch.nn.Conv2d(in_dim, out_dim, kernel_size=3, stride=1, padding=1)
        self.gate = torch.nn.Parameter(torch.zeros(out_dim))
        self.conv_in.requires_grad_(False) #do not update weight of the exisitng one.

    def set_conv_in(self, conv_in):
    ...
    def forward(self, x):
        ...
        mask = x[:,4:5]
        cond = x[:,5:]
        y1 = self.conv_in(latents)
        y2 = self.conv_cond(cond)
        y = y1 + (self.gate[None,:,None,None] * y2)
        return y
