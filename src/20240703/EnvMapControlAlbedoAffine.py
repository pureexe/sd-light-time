...
class EnvMapControlAlbedoAffine(L.LightningModule):

    def __init__(self, controlnet_path = "lllyasviel/sd-controlnet-depth", learning_rate=1e-3, face100_every=20, guidance_scale=7.5, envmap_embedder="vae", *args, **kwargs) -> None:
        super().__init__()
    ...
    def load_controlnet(self, controlnet_path):
        controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=MASTER_TYPE).to('cuda')
    ...
    def get_face_albedo(self, face_id):
        FACE_PATH = "datasets/face/face2000_single/albedo/{}/{}.png"
        dir_id = face_id // 1000
        dir_id = dir_id * 1000
        face_path = FACE_PATH.format(f"{dir_id:05d}", f"{face_id:05d}")
        img = Image.open(face_path).convert('RGB').resize((512,512))
    ...
    def generate_video_light(self):
        ...
        for face_id in range(TOTAL_FACE):
            ...
            control_image = self.get_face_albedo(face_id)
    ...
    def generate_tensorboard(self, batch, batch_idx, is_save_image=False):
        ...
        # control_image = estimate_scene_depth(face_image, depth_estimator=self.depth_estimator)
        face_ids = [1237, 700, 495, 1923, 1133, 715, 497, 644, 554, 1173]
        face_id = face_ids[batch_idx % len(face_ids)]
        control_image = self.get_face_albedo(face_id)
