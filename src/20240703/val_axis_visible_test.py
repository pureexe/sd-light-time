...
for chk_pt in CHK_PTS:
    ...
    if ENV_MODE == "visible_test":
        print("visible_test")
        prompt_path = "datasets/face/face2000_single/prompts.json"
        from EnvmapAffineTestDataset import EnvmapAffineTestDataset
        for axis_name in ['top_left']:
            trainer = L.Trainer(max_epochs=1000, precision=16, check_val_every_n_epoch=1, default_root_dir=f"output/20240703/val_axis_visible_test/{NAMES[VERSION]}/{LRS[VERSION]}/chk{chk_pt}/{axis_name}")
            val_dataset = EnvmapAffineTestDataset(root_dir=f"datasets/validation/visible_test/{axis_name}", dataset_multiplier=10, prompt_path=prompt_path)
