...
class SDRelightWithChromeball(L.LightningModule):
    ...
    def compute_train_loss(self, batch, batch_idx, timesteps=None, seed=None):
        ...
        ball_latents = self.pipe.scheduler.step(model_pred, timesteps, noisy_latents).prev_sample
        ball_timesteps = timesteps -1 
        ...
        # SECOND PASS 
        TIMESTEP = 800
        ball_timesteps = self.get_timesteps(TIMESTEP, batch_size, device)
        ball_x0_noisy =  self.pipe.scheduler.add_noise(ball_x0, noise, ball_timesteps)

        if ball_guidance_scale > 1.0:
            ball_latents_input = torch.cat([ball_x0_noisy, ball_x0_noisy])
        else:
            ball_latents_input = ball_x0_noisy
