...
GUIDANCE_SCALE = 7.0
NULL_STEP = 10
EXP_NAME = f"latentinput_{NUM_INFERENCE_STEPS}_4"
...
def main():
    ...
    # add 
    pipe._callback_tensor_inputs = ["latents", "prompt_embeds", "latent_model_input", "timestep_cond", "added_cond_kwargs", "extra_step_kwargs", "noise_pred"]
    ...
    def callback_apply_nulltext_OLD(pipe, step_index, timestep, callback_kwargs):
    ...
    def callback_apply_nulltext(pipe, step_index, timestep, callback_kwargs):
        ...
        with torch.enable_grad():
            if True:
                ...
                # predict next latents
                predict_latents = pipe.scheduler.step(noise_pred, timestep, latents, **...['extra_step_kwargs'] , return_dict=False)[0]
