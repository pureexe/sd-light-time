...
def sample(
    ...
    for i in tqdm(range(start_step, num_inference_steps)):
        ...
        intermediate_timesteps.append(t[None])

    # Post-processing
    images = pipe.decode_latents(latents)
    images = pipe.numpy_to_pil(images)

    return images, torch.cat(intermediate_latents), torch.cat(intermediate_timesteps)
...
with torch.inference_mode():
    ...
    start_step = 0
    output_image, denoised_latents,  denoised_timesteps = sample(
        input_image_prompt,
        start_latents=inverted_latents[-(start_step + 1)][None],
        start_step=start_step,
        num_inference_steps=TOTAL_STEP-1,
        guidance_scale=GUIDANCE_SCALE,
    )
