...
class DDIMUnsplashLiteDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, batch_idx):
        ...
        try:
            ...
            try:
                control_depth = self.transform_depth(self.get_image(name,"control_depth", 512, 512))
            except:
                control_depth = torch.zeros(3, 512, 512)
            
            try:
                chromeball = self.get_image(name,"chromeball", 512, 512)
            except:
                chromeball = torch.zeros(3, 512, 512)

            try:
                control_normal = self.transform_normal(self.get_image(name,"control_normal", 512, 512))
            except:
                control_normal = torch.zeros(3, 512, 512)

            try:
                control_normal_bae = self.transform_normal(self.get_image(name,"control_normal_bae", 512, 512))
            except:
