...
def main():
    ...
    for image_id in tqdm(image_ids[args.start:args.end]):
        for direction in split:
            ...
            output_dir = f"output/20241102_sd35med/{direction}_6_15"
            os.makedirs(output_dir, exist_ok=True)
            output_path = os.path.join(output_dir, f"{image_id:05d}.jpg")
            os.system(f"python src/20241102/sd35_inference.py --start_step 6 --end_step 15 --image_path {image_path} --output_path {output_path} --source_prompt \"{PROMPT_FROM}\" --prompt \"{PROMPT_TO}\"")
