...
def decode_imgs(latents, pipeline, output_type='pil'):
...
def main():
    ...
    parser.add_argument('--eta_base', type=float, default=0.8, help='Eta parameter for interpolated_denoise')
    ...
    parser.add_argument('--start_step', type=int, default=9, help='Start step for eta values, 0-based indexing, closed interval')
    parser.add_argument('--end_step', type=int, default=21, help='End step for eta values, 0-based indexing, open interval')
    ...
    pipe = StableDiffusion3Pipeline.from_pretrained(args.model_path, torch_dtype=DTYPE, text_encoder_3=None, tokenizer_3=None)
    ...
    print(f"Saved output image to {args.output_path} with parameters: eta_base={args.eta_base}, start_step={args.start_step}, end_step={args.end_step}, guidance_scale={args.guidance_scale}, seed={args.seed}")
