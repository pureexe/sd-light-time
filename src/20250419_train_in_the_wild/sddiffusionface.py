    ...
    def generate_tensorboard(self, batch, batch_idx, is_save_image=False, is_seperate_dir_with_epoch=False):   
        ...
        for target_idx in range(len(batch['target_image'])):
            ...
            if is_save_image:
                ...
                # save sd image file
                os.makedirs(f"{log_dir}/{epoch_text}sd_output", exist_ok=True)
                torchvision.utils.save_image(sd_image, f"{log_dir}/{epoch_text}sd_output/{filename}.png")
                os.chmod(f"{log_dir}/{epoch_text}sd_output", 0o777)
                os.chmod(f"{log_dir}/{epoch_text}sd_output/{filename}.png", 0o777)
                # save psnr to file
