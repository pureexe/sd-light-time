...
def cartesian_to_spherical(cartesian_coordinates):
    ...
    Returns:
        ...
        coordinate (r, theta, phi). # theta is in range [-pi, pi], # phi is in range [0,pi] 
    """
...
def main():
    ...
    # swap x and z 
    normal_image[:,:,1], normal_image[:,:,2] =  normal_image[:,:,2], normal_image[:,:,1] 
    ...
    spherical_coords = cartesian_to_spherical(normal_map[0].permute(1,2,0).numpy())
    ...
    # theta actually need range [0, 2pi]
    theta_phi[...,0]  = theta_phi[...,0]  * 2 * np.pi
    # phi is in range [-pi/2,pi/2] 
    theta_phi[...,1] = ((theta_phi[...,1] * 2.0) - 1.0) * np.pi / 2
