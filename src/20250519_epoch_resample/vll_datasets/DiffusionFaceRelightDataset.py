...
ACCEPT_EXTENSION = ['jpg', 'png', 'jpeg', 'exr', 'npy', 'npz']
IMAGE_DIR = "images"
IS_DEBUG = True
...
class DiffusionFaceRelightDataset(torch.utils.data.Dataset):
    ...
    def setup_transform(self):
        ...
        max_shading = self.max_shading
        class MaxShadingTransform(object):
            def __call__(self, img):
                # Ensure the image is a tensor
                if not isinstance(img, torch.Tensor):
                    raise TypeError("Expected image to be a torch.Tensor")
                img =  img / max_shading # range [0,1]
                img = img * 2 - 1 # range [-1,1]
                return img
                

        control_transform_list = [
            torchvision.transforms.Resize(512,  antialias=True),  # Resize the image to 512x512
        ]
        if max_shading > 0.0:
            control_transform_list.append(MaxShadingTransform())
            
        self.transform['control'] = torchvision.transforms.Compose(control_transform_list)
            ...
            if os.path.exists(image_path):
                if ext in ['exr', 'npy', 'npz']:
                    if ext == 'exr':
                        ...
                        image = image[image.files[0]]  # [H,W,C]
