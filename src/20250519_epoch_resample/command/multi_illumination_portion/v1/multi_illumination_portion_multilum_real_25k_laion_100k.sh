...
SHARD_VERSION="multilum_real_25k_laion_100k"
MODEL_DIR="stable-diffusion-v1-5/stable-diffusion-v1-5"
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250510_webdataset_support/controlnet/multi_illumination_portion/v1/${SHARD_VERSION}"
TRAIN_DATA_DIR="/ist/ist-share/vision/pakkapon/relight/sd-light-time/src/20250510_webdataset_support/command/multi_illumination_portion/v1/shardlist/${SHARD_VERSION}.txt"
DATA_PER_EPOCH="124625"
NUM_EPOCH="1000"
NUM_STEPS=$((DATA_PER_EPOCH * NUM_EPOCH / (BATCH_SIZE * GRAD_ACCUM) )) # 1000 EPOCH
COMMAND="accelerate launch train_controlnet.py --pretrained_model_name_or_path=${MODEL_DIR} --output_dir=${OUTPUT_DIR} --train_data_dir=${TRAIN_DATA_DIR} --resolution=${RESOLUTION} --learning_rate=${LEARNING_RATE} --train_batch_size=${BATCH_SIZE} --gradient_accumulation_steps=${GRAD_ACCUM} --checkpointing_steps ${CHECK_EVERY} --max_train_steps ${NUM_STEPS} --use_8bit_adam --mixed_precision fp16 --dataloader_num_workers ${NUM_WORKER} --estimated_dataset_length ${DATA_PER_EPOCH}  --resume_from_checkpoint latest"
