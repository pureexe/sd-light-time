#!/bin/bash
#SBATCH --error=output/logs/error/%j_1e-5max6_realgt_err.txt   # STDOUT error is written in slurm.out.JOBID
#SBATCH --output=output/logs/output/%j_1e-5max6_realgt_out.txt  # STDOUT output is written in slurm.err.JOBID
#SBATCH --job-name=1e-5max32realreal          #            
...
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250519_epoch_resample/controlnet/multi_illumination_real_image_real_shading/v0_hf_max6/${LEARNING_RATE}"
DATASET_PATH="/pure/f1/datasets/multi_illumination/real_image_real_shading/v0/hf/MultiRealImageRealShading/MultiRealImageRealShading.py"
COMMAND="accelerate launch train_controlnet.py --pretrained_model_name_or_path=${MODEL_DIR} --output_dir=${OUTPUT_DIR} --dataset_name=${DATASET_PATH} --resolution=${RESOLUTION} --learning_rate=${LEARNING_RATE} --train_batch_size=${BATCH_SIZE} --gradient_accumulation_steps=${GRAD_ACCUM} --checkpointing_steps ${CHECK_EVERY} --max_train_steps ${NUM_STEPS} --use_8bit_adam --mixed_precision fp16 --dataloader_num_workers ${NUM_WORKER}  --resume_from_checkpoint latest --max_shading_value 6"
