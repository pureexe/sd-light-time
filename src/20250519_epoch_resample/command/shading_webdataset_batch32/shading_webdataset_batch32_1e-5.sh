...
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250510_webdataset_support/controlnet/shading_webdataset_batch${TOTAL_BATCH}/v1/${LEARNING_RATE}"
...
COMMAND="accelerate launch train_controlnet.py --pretrained_model_name_or_path=${MODEL_DIR} --output_dir=${OUTPUT_DIR} --train_data_dir=${TRAIN_DATA_DIR} --resolution=${RESOLUTION} --learning_rate=${LEARNING_RATE} --train_batch_size=${BATCH_SIZE} --gradient_accumulation_steps=${GRAD_ACCUM} --checkpointing_steps ${CHECK_EVERY} --max_train_steps ${NUM_STEPS} --use_8bit_adam --mixed_precision fp16 --dataloader_num_workers ${NUM_WORKER}"

echo ${COMMAND}

CUDA_VISIBLE_DEVICES=1 singularity exec --bind /pure:/pure --bind /ist:/ist --nv --env HF_HUB_CACHE=/ist/ist-share/vision/huggingface/hub/ --env HUB_HOME=/ist/ist-share/vision/huggingface/ ${CONTAINER} ${COMMAND}
