...
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250519_epoch_resample/controlnet/multi_illumination_lstsq_image_lstsq_shading/v1/${LEARNING_RATE}"
TRAIN_DATA_DIR="/pure/f1/datasets/multi_illumination/lstsq_image_lstsq_shading/v1/train/train-{0000..024}.tar"
ESTIMATE_LENGTH=24625
COMMAND="accelerate launch train_controlnet.py --pretrained_model_name_or_path=${MODEL_DIR} --output_dir=${OUTPUT_DIR} --train_data_dir=${TRAIN_DATA_DIR} --resolution=${RESOLUTION} --learning_rate=${LEARNING_RATE} --train_batch_size=${BATCH_SIZE} --gradient_accumulation_steps=${GRAD_ACCUM} --checkpointing_steps ${CHECK_EVERY} --max_train_steps ${NUM_STEPS} --use_8bit_adam --mixed_precision fp16 --dataloader_num_workers ${NUM_WORKER}  --resume_from_checkpoint latest --estimated_dataset_length ${ESTIMATE_LENGTH}"
