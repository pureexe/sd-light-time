#!/bin/bash
#SBATCH --error=output/logs/error/%j_max32realreal_err.txt   # STDOUT error is written in slurm.out.JOBID
#SBATCH --output=output/logs/output/%j_realreal_out.txt  # STDOUT output is written in slurm.err.JOBID
#SBATCH --job-name=1e-4max32realreal          #            
...
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250519_epoch_resample/controlnet/multi_illumination_real_image_real_shading/v0_hf_max32/${LEARNING_RATE}"
