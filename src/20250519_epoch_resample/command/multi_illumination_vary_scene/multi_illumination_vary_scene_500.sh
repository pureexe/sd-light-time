#!/bin/bash
#SBATCH --error=output/logs/error/%j_vary500_err.txt   # STDOUT error is written in slurm.out.JOBID
#SBATCH --output=output/logs/output/%j_vary500_out.txt  # STDOUT output is written in slurm.err.JOBID
#SBATCH --job-name=500vary          #            
...
OUTPUT_DIR="/pure/t1/checkpoints/sd-light-time/20250519_epoch_resample/controlnet/multi_illuination_vary_scene/v1/num_scene_500"
DATASET_PATH="/pure/f1/datasets/multi_illumination/lstsq_image_lstsq_shading/v0/hf/MultiLstShadingScene500/MultiLstShadingScene500.py"
