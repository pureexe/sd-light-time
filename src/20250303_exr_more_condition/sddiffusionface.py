...
class SDDiffusionFace(L.LightningModule):
    ...
    def generate_tensorboard_NEW(self, batch, batch_idx, is_save_image=False, is_seperate_dir_with_epoch=False):   
        try:
            ...
            if "image" in pipe_args:
                if pipe_args["image"].shape[1] % 3 == 0:
                    for i in range(pipe_args["image"].shape[1] // 3):
                        ctrl_image = pipe_args["image"][:, (i)*3:(i+1)*3]
                        if ctrl_image.max() > 1.0 and ctrl_image.min() >= 0.0: # incase range [0, inf] we reduce to [0,1]
                            ctrl_image = ctrl_image / ctrl_image.max() # support display exr image
                        else:
                            ctrl_image = (ctrl_image + 1.0) / 2.0
...
class SDShadingwithDivAlbedo(SDDiffusionFace):
    def setup_sd(self, sd_path="runwayml/stable-diffusion-v1-5", controlnet_path=None):
        # load main UNet
        unet = UNet2DConditionModel.from_pretrained(sd_path, subfolder='unet')

        # create controlnet from unet. Unet take 6 channel input first 3 channel will be fore shading, other 3 will be fore albedo
        controlnet = ControlNetModel.from_unet(unet, conditioning_channels=6) 
    ...
    def get_control_image(self, batch, array_index=None, query_false_shading=False):
        key_shading = 'source_shading'
        key_image = 'source_image'
        if not key_shading in batch :
            key_shading = 'shading'

        shading = batch[key_shading] # range [0,inf]
        source_image = batch[key_image] # by default: range [-1,1]
        # rescale to range [0,1]
        source_image  = (source_image + 1.0) /2.0
        assert source_image.min() >= 0.0 and source_image.max() <= 1.0
        albedo = source_image / (shading + 1e-6) # 1e-6 is to avoid div0
        return torch.cat([shading, albedo],dim=1)
...
class SDShadingwithEdge(SDDiffusionFace):
    def setup_sd(self, sd_path="runwayml/stable-diffusion-v1-5", controlnet_path=None):
        # load main UNet
        unet = UNet2DConditionModel.from_pretrained(sd_path, subfolder='unet')

        # create controlnet from unet. Unet take 4 channel input first 3 channel will be fore shading and the last channel will be the edge
    ...
    def get_control_image(self, batch, array_index=None, query_false_shading=False):
        ...
        source_image  = (source_image + 1.0) /2.0
        assert source_image.min() >= 0.0 and source_image.max() <= 1.0
        edge = sobel_edge_detection(source_image)
        condition =  torch.cat([shading, edge],dim=1)
        return condition