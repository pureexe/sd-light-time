...
class UNet2DLeftRightAffineConditionModel(UNet2DConditionModel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def set_direction(self, direction):
        for block_id in range(3):
            self.down_blocks[block_id].downsamplers[0].set_direction(direction)
        for block_id in range(3):
            self.up_blocks[block_id].upsamplers[0].set_direction(direction)
...
class AffineBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states, *args, **kwargs):
        out = hidden_states * self.multipiler[self.direction_id][None] + self.bias[self.direction_id][None]
        return out