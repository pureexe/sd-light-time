...
class DDIMArrayEnvDataset(BaseRelightDataset):
    ...
    def get_item(self, idx, batch_idx):
        ...
        for envmap_name in self.envmap_index[idx]:
            output['target_ldr_envmap'].append(self.transform['envmap'](self.get_image(envmap_name,"env_ldr", 256, 256)))
            output['target_norm_envmap'].append(self.transform['envmap'](self.get_image(envmap_name,"env_under", 256, 256)))
