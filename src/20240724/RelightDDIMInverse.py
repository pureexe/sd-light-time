...
from RelightEnvmapDataset import log_map_to_range
...
class RelightDDIMInverse(L.LightningModule):
    def __init__(self, 
            learning_rate=1e-3,
            face100_every=20,
            guidance_scale=3.0,
            envmap_embedder="dino2",
            *args, 
            **kwargs
        ) -> None:
        super().__init__()
        ...
        #mlp_in_channel = self.build_architecture(envmap_embedder)
    ...
    # def build_architecture(self, envmap_embedder):
    #     # load envmap_embedder
    #     self.envmap_embbeder_name = envmap_embedder
    #     if envmap_embedder == "dino2":
    #         mlp_in_channel = 768*2
    ...
    def get_vae_features(self, images):
        ...
        with torch.inference_mode():
            ...
            emb = self.pipe.vae.encode(images).latent_dist.sample() # note that we didn't multiply by scaling factor here
            flattened_emb = emb.view(emb.size(0), -1)
            return flattened_emb
    ...
    def generate_tensorboard(self, batch, batch_idx, is_save_image=False):
        ...
        with torch.inference_mode():
            z0_noise = self.pipe.vae.encode(batch['pixel_values']).latent_dist.sample() * self.pipe.vae.config.scaling_factor
            print(batch['text'])
        ...
        if is_save_image:
            ...
            torchvision.utils.save_image(image, f"{self.logger.log_dir}/with_groudtruth/{batch['name'][0]}_{batch['env_name'][0]}.png")
    ...
    def validation_step(self, batch, batch_idx):
        # we don't run validation at a moment, but probably require to run soon
