...
def log_validation(
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        # @TODO: PURE-edit: need for supprot EXR
        ...
        for _ in range(args.num_validation_images):
            with inference_ctx:
                # @TODO PURE-edit: need proper inversion support
                image = pipeline(
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Value use for normalize albedo into range [0,1]. "
            "leave it empty or set value <= 0.0 to prevent this normalization"
        )
    )
    ...
    parser.add_argument(
        "--num_controlnet_conditioning_channels",
        type=int,
        default=3,
        help=(
            "number of controlnet conditioning channel, default is 3"
        )
    )
...
def make_train_dataset(args, tokenizer, accelerator):
    if is_web_dataset(args):
        return make_train_dataset_webdataset(args, tokenizer, accelerator)
    else:
        return make_train_dataset_huggingface(args, tokenizer, accelerator)

def make_train_dataset_webdataset(args, tokenizer, accelerator):
    """
    USE webdataset for superior performance (i guess?)
    """
    ...
    def condition_transform(item):
        item = torch.from_numpy(item).permute(2,0,1).half()
    ...
    dataset = (
        wds.WebDataset(train_data_dir, resampled=False)        
        .select(lambda sample: all(k in sample for k in ["jpg", "npz", "txt"]))
        .to_tuple("jpg", "npz", "txt")
        .map(lambda data_tuple: {
            "pixel_values": image_transform(Image.open(io.BytesIO(data_tuple[0])).convert("RGB")),
            "conditioning_pixel_values": condition_transform(decode_npz(data_tuple[1])),
            "input_ids": tokenize(data_tuple[2]),
        })
    )
...
def make_train_dataset_huggingface(args, tokenizer, accelerator):
    ...
    def preprocess_train(examples):
        ...
        for idx, image in enumerate(examples[conditioning_image_column]):
            current_meta = []
            metas = json.loads(image)
            for key_name in ['shading', 'albedo']:
                image =  metas.get(key_name, None)
                if image is None:
                    continue
                # check if it pil 
                if isinstance(image, Image.Image):
                    # if it pil, we do regular one 
                    cond_image = image.convert("RGB")
                    cond_image = conditioning_image_transforms(cond_image)
                    current_meta.append(cond_image)
                else:
                    ...
                    if args.max_shading_value > 0:
                        if key_name == 'shading':
                            divder = args.max_shading_value
                        elif key_name == 'albedo':
                            divder = args.max_albedo_value
                        cond_image = cond_image / divder
                    ...
                    current_meta.append(cond_image)
            current_meta = torch.cat(current_meta, dim=0)
            
            conditioning_images.append(current_meta)
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_unet(unet, conditioning_channels=args.num_controlnet_conditioning_channels)
    ...
    is_webdataset_format = is_web_dataset(args)

    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=not is_webdataset_format,
        collate_fn=None if is_webdataset_format else collate_fn,
