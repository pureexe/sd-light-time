#!/bin/bash
#SBATCH --error=output/logs/error/%j_1e-5max6_realgtalbedo_err.txt   # STDOUT error is written in slurm.out.JOBID
#SBATCH --output=output/logs/output/%j_1e-5max6_realgtalbedo_out.txt  # STDOUT output is written in slurm.err.JOBID
#SBATCH --job-name=1e-5max6realgtalbedo         #            
