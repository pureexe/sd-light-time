...
#SBATCH --job-name=5e-5mp          # Job name
...
LEARNING_RATE="5e-5"
COMMAND="python src/20250419_train_in_the_wild/train.py -lr ${LEARNING_RATE} --guidance_scale 1.0 --network_type sd_no_bg --batch_size 8 -c 1 --feature_type clip --shadings_dir shadings_marigold --backgrounds_dir images --images_dir images -dataset /data/pakkapon/datasets/laion-shading/v3/train -dataset_split /data/pakkapon/datasets/laion-shading/v3/train/index/100k_marigold.json -dataset_val /data/pakkapon/datasets/laion-shading/v3/test -dataset_val_split /data/pakkapon/datasets/laion-shading/v3/test/index/main_v2.json  --val_check_interval 0.2  -specific_prompt _"
