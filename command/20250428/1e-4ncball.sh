...
#SBATCH --job-name=1e-4ncball          # Job name: 1e-4 no clip multi-ilumn  #110437
...
COMMAND="python src/20250419_train_in_the_wild/train.py -lr ${LEARNING_RATE} --guidance_scale 1.0 --network_type ${NETWORK_TYPE} --batch_size 8 -c 5 --feature_type clip --shadings_dir control_shading_from_fitting_v3_exr --backgrounds_dir control_render_from_fitting_v2 --images_dir control_render_from_fitting_v2 -dataset /data/pakkapon/datasets/multi_illumination/spherical/train  -dataset_val /data/pakkapon/datasets/multi_illumination/spherical/val -dataset_val_split /data/pakkapon/datasets/multi_illumination/spherical/split-train-relight-img5.json"
